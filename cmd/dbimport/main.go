...
func main() {
	...
	files = append(files,
		...
		"db/reviews.sql",
		// Always run: review_photos table (FKs reviews.id). Idempotent.
		"db/review-photos.sql",
		// Always run: buyer-context snapshot columns + order_id ON DELETE SET NULL.
		"db/review-context.sql",
